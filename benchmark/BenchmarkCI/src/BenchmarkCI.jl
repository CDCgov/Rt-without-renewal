...
function versionof(pkg::Module)
    ...
    try
        ...
        @error("Failed to read version from `$projpath`",
...
"""
judge(; target = nothing, baseline = "origin/master", kwargs...) = judge(
    target, baseline; kwargs...)
...
function git_commit_info(rev::AbstractString, benchmarkdir::AbstractString)
    ...
    info = Dict(
        ...
        :git_commit_date => strip(read(`$git show --no-patch --format=%cI $rev`, String))
    )
end
...
function _loadciresult(workspace::AbstractString = DEFAULT_WORKSPACE)
...
"""
postjudge(workspace::AbstractString = DEFAULT_WORKSPACE; kwargs...) = postjudge(
    _loadjudge(workspace); kwargs...)
...
function printresultmd(io, ciresult)
    ...
    md = try
        ...
        @error("""`show(_, "text/markdown", CpuId.cpuinfo())` failed""",
            exception=(err, catch_backtrace()))
        nothing
    end
...
"""
displayjudgement(workspace::AbstractString = DEFAULT_WORKSPACE) = displayjudgement(_loadjudge(workspace))
