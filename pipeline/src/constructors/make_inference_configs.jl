...
"""
Create inference configurations for the given pipeline. This is the example method,
which only returns a randomly selected inference configuration.
...
"""
function make_inference_configs(pipeline::EpiAwareExamplePipeline)
    gi_param_dict = make_gi_params(pipeline)
    namemodel_vect = make_epiaware_name_latentmodel_pairs(pipeline)
    igps = make_inf_generating_processes(pipeline)
    obs = make_observation_model(pipeline)
