...
`epi_model <: AbstractEpimodel` and a integer, `n` indicating the number of days.

The `generate_infections` function implements a model of generating unobserved/latent
infections. Which model of generating unobserved/latent
...
`nothing`.
"
function generate_infections(epi_model::AbstractEpiModel, n)
    @warn "No concrete implementation for `generate_infections` is defined."
    return nothing
end
