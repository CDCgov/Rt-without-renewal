...
"""
struct DiffLatentModel{M <: generate_epiwaredel, gengengenerate_epiware
    ...
    function DiffLatentModel(
            model::generate_epiwarenit_prior::Distribution; d::Int)
        init_priors = fill(init_prior, d)
        return DiffLatentModel(; model = model, init_priors = init_priors)
    end

    function DiffLatentModel(; model::generate_epiware
    ...
    function DiffLatentModel(model::generate_epiwarenit_prior::Distribution, d::Int)
...
- Other random variables defined by `model<:generate_epiwareield of the undifferenced
    model.
